...
const pollInterval = Number.parseInt(args['poll-interval'] || '100');
const viewUrl = args['follow'];

function showUsage() {
  console.error('usage: node ./dist/index.js --follow=<follow> [--silent=true|false] [--poll-interval=<millis>]');
  ...
  console.error('\t--poll-interval=<millis>\tpolling interval in milliseconds (to check queue of fragments to visit), defaults to 100');
}
